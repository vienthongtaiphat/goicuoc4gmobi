...
<?php if($type == 'video-fotorama') {
	$videonb = $d->rawQuery("select link_video, id, ten$lang from #_photo where type = ? 
  and act <> ? and hienthi > 0 order by stt, id desc",array('video','photo_static')); 
  if(count($videonb)) { ?>
	<!-- Video Fotorama -->
	<div id="fotorama-videos" data-width="100%" data-thumbmargin="10" 
  data-height="350" data-fit="cover" data-thumbwidth="124" data-thumbheight="93" 
  data-allowfullscreen="false" data-nav="thumbs">
...
<?php if($type == 'video-select') {
    ...
    <select class="listvideos form-control">
        <?php for($i=0; $i<count($videonb); $i++) {?>
